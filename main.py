...
def final_display():
    ...
    for i in SAMPLES:
        filename="pipeline1_ref_pangolin_report/"+"pangolin_"+i+'.csv'
    ...
    for i in SAMPLES:
        filename="pipeline2_deNovo_pangolin_report/"+"pangolin_"+i+'.csv'
...
# function of generating midresult
def bwa_index():
    pass
...
# function of running snakemake of first pipeline(use reference-guide assembly)
def run_snakemake():
    ...
    pangolinstr=""
    for i in SAMPLES:
        pangolinstr+=pangolinexec("pipeline1_ref_result/{0}_consensus.fa".format(i),"pipeline1_ref_pangolin_report/","pangolin_{0}.csv".format(i))
    condastr="bash -c \" {0} {1} {2} {3}\"".format(envstr,bwa,snakemakestr,pangolinstr)
    print(condastr)
    os.system(condastr)

# function of running snakemake of second pipeline(use de novo assembly)
def run_snakemake2():
    envstr="source ~/mambaforge/etc/profile.d/conda.sh;conda activate pangolin;"
    ...
    pangolinstr=""
    for i in SAMPLES:
        pangolinstr+=pangolinexec("pipeline2_deNovo_ragtag_output/scaffold/{0}/ragtag.scaffold.fasta".format(i),"pipeline2_deNovo_pangolin_report","pangolin_{0}.csv".format(i))
    condastr="bash -c \" {0} {1} {2} {3}\"".format(envstr,bwa,snakemakestr,pangolinstr)
    print(condastr)
    os.system(condastr)
